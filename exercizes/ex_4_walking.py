...
USE_PROXQP = 1
USE_OSQP = 0
# retrieve and save the matrices describing the QP problem of each time step
GET_QP_DATA = 1
...
for i in range(-N_pre, N+N_post):
    ...
    if GET_QP_DATA:
        qp_data_list.append((tsid_biped.solver.qpData, sol.x))
...
if GET_QP_DATA:
    ...
    H = qp_data_list[0][0].H
    CE = qp_data_list[0][0].CE
    CI = qp_data_list[0][0].CI
    g = qp_data_list[0][0].g
    ce0 = qp_data_list[0][0].ce0
    x = qp_data_list[0][1]

    if USE_PROXQP:
        ub = qp_data_list[0][0].ub
        lb = qp_data_list[0][0].lb
        qpData = (H, CE, CI, g, ce0, ub, lb, x)
    else:
        ci0 = qp_data_list[0][0].ci0
        qpData = (H, CE, CI, g, ce0, ci0, x)
    
    c = 1

    def save_and_reset(save_dict, qpData, c):
        # print(f"[save_and_reset] c: {c}")
        if USE_PROXQP:
            H, CE, CI, g, ce0, ub, lb, x = qpData
        else:
            H, CE, CI, g, ce0, ci0, x = qpData

        H = H.reshape(c, -1, H.shape[-1])
        CE = CE.reshape(c, -1, CE.shape[-1])
        CI = CI.reshape(c, -1, CI.shape[-1])
        g = g.reshape(c, -1)
        ce0 = ce0.reshape(c, -1)
        x = x.reshape(c, -1)
        ...
        save_dict[f"H_{i}"] = H
        save_dict[f"CE_{i}"] = CE
        save_dict[f"CI_{i}"] = CI
        save_dict[f"g_{i}"] = g
        save_dict[f"ce0_{i}"] = ce0
        save_dict[f"x_{i}"] = x

        if USE_PROXQP:
            ub = ub.reshape(c, -1)
            lb = lb.reshape(c, -1)
            save_dict[f"ub_{i}"] = ub
            save_dict[f"lb_{i}"] = lb

        else:
            ci0 = ci0.reshape(c, -1)
            save_dict[f"ci0_{i}"] = ci0

        H = qp_data_list[i + 1][0].H
        CE = qp_data_list[i + 1][0].CE
        CI = qp_data_list[i + 1][0].CI
        g = qp_data_list[i + 1][0].g
        ce0 = qp_data_list[i + 1][0].ce0
        x = qp_data_list[i + 1][1]

        if USE_PROXQP:
            ub = qp_data_list[i + 1][0].ub
            lb = qp_data_list[i + 1][0].lb
            qpData = (H, CE, CI, g, ce0, ub, lb, x)
        else:
            ci0 = qp_data_list[i + 1][0].ci0
            qpData = (H, CE, CI, g, ce0, ci0, x)

        c = 1

        return save_dict, qpData, c

    for i in range(len(qp_data_list) - 1):
        try:
            H = np.concatenate((H, qp_data_list[i + 1][0].H))
            CE = np.concatenate((CE, qp_data_list[i + 1][0].CE))
            CI = np.concatenate((CI, qp_data_list[i + 1][0].CI))
            g = np.concatenate((g, qp_data_list[i + 1][0].g))
            ce0 = np.concatenate((ce0, qp_data_list[i + 1][0].ce0))
            x = np.concatenate((x, qp_data_list[i + 1][1]))

            if USE_PROXQP:
                ub = np.concatenate((ub, qp_data_list[i + 1][0].ub))
                lb = np.concatenate((lb, qp_data_list[i + 1][0].lb))
                qpData = (H, CE, CI, g, ce0, ub, lb, x)

            else:
                ci0 = np.concatenate((ci0, qp_data_list[i + 1][0].ci0))
                qpData = (H, CE, CI, g, ce0, ci0, x)
