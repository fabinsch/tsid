...
namespace tsid
{
  namespace solvers
  {
    ...
    m_solver()  // dim of primal var needs to be strictly positiv
    {
      m_n = 0;
      m_neq = 0;
      m_nin = 0;
    ...
    const HQPOutput & SolverOSQP::solve(const HQPData & problemData)
    {
      ...
      m_solver.data()->setLowerBound(m_qpData.ci_lb);
      m_solver.data()->setUpperBound(m_qpData.ci_ub);
    ...
    bool SolverOSQP::setMaximumIterations(unsigned int maxIter)
    {
      ...
      m_solver.settings()->setMaxIteration(maxIter);
      return true;
    }
